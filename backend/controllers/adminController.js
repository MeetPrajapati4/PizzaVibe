...
export const getEnhancedStats = async (req, res, next) => {
  try {
    ...
    // Top selling pizzas
    const topPizzasRaw = await OrderItem.findAll({
      attributes: [
        'name',
    ...
    const topPizzas = topPizzasRaw.map(p => ({
      name: p.name,
      totalSold: parseInt(p.dataValues.totalSold, 10),
    ...
    // This query uses standard SQL DATE function which is widely supported, including Postgres
    const dailyRevenueRaw = await Order.findAll({
      attributes: [
    ...
    const dailyRevenue = dailyRevenueRaw.map(d => ({
      ...
      revenue: parseFloat(d.dataValues.revenue),
      orders: parseInt(d.dataValues.orders, 10)
    }));
